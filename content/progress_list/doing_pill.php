...
<?php foreach ($get_task_doing as $tdoing) : ?>
  <a href="./details-progress.php?tid=<?= $tdoing['id'] ?>&id=<?= $_GET['id'] ?>&bid=<?= $_GET['bid'] ?>" class="focus:ring-4 focus:outline-none bg-white shadow focus:ring-gray-200 flex items-center justify-between hover:bg-gray-100 text-sm w-10/12 px-3 py-2 border-2 border-gray-200 my-1.5 rounded-lg">
    <?= $tdoing['task_name'] ?>

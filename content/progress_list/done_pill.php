...
<?php foreach ($get_task_done as $tdone) : ?>
  <a href="./details-progress.php?tid=<?= $tdone['id'] ?>&id=<?= $_GET['id'] ?>&bid=<?= $_GET['bid'] ?>" class="focus:ring-4 focus:outline-none bg-white shadow focus:ring-gray-200 flex items-center justify-between hover:bg-gray-100 w-10/12 text-sm px-3 py-2 border-2 border-gray-200 my-1.5 rounded-lg">
    <?= $tdone['task_name'] ?>

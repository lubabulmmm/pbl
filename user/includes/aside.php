...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 px-4 transition-transform -translate-x-full bg-white border-r shadow-sm border-gray-200 sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-1 pb-6 overflow-y-auto bg-white flex flex-col justify-between align-center">
      <ul class="space-y-2 font-medium">
         <li>
            <a href="/PBL/user/dashboard.php" class="flex items-center hover:bg-blue-100 py-2 px-3 font-medium text-blue-800 text-sm rounded-lg hover:bg-white-200 group">
         ...
         <li>
            <button type="button" class="flex items-center w-full hover:bg-blue-100 py-2 px-3 font-medium text-blue-800 text-sm transition duration-75 rounded-lg group " aria-controls="dropdown-example" data-collapse-toggle="dropdown-example">
            ...
            <ul id="dropdown-example" class="hidden py-2 space-y-2 duration-300">
               <?php foreach ($user_projects_pm as $upm) : ?>
                  <li>
                     <a href="/PBL/user/projects.php?bid=<?= $upm['bunch_id'] ?>&id=<?= $upm['project_id'] ?>" class="flex items-center w-full hover:bg-blue-100 py-2 px-3 font-medium text-blue-800 text-sm transition duration-75 rounded-lg pl-11 group "><?= $upm['nama_proyek'] ?></a>
                  </li>
               <?php endforeach; ?>
               <?php foreach ($user_projects_member as $u_member) : ?>
                  <li>
                     <a href="/PBL/user/projects.php?bid=<?= $u_member['bunch_id'] ?>&id=<?= $u_member['project_id'] ?>" class="flex items-center w-full hover:bg-blue-100 py-2 px-3 font-medium text-blue-800 text-sm transition duration-75 rounded-lg pl-11 group "><?= $u_member['nama_proyek'] ?></a>
                  </li>
               <?php endforeach; ?>
            </ul>
         </li>
         <li>
            <a href="/PBL/user/profile.php" class="flex items-center hover:bg-blue-100 py-2 px-3 font-medium text-blue-800 text-sm rounded-lg  group">
         ...
         <li>
            <a href="/PBL/user/showcase.php" class="flex items-center hover:bg-blue-100 py-2 px-3 font-medium text-blue-800 text-sm rounded-lg  group">
               <svg class="flex-shrink-0 w-4 h-4 text-blue-800 text-sm transition duration-75" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
         ...
         <li>
            <a href="/PBL/user/logout.php" class="flex items-center hover:bg-blue-100 py-2 px-3 font-medium text-blue-800 text-sm rounded-lg  group">
